...
export default {
  ...
  desc: 'Emits a CRYO pulse that chips up to 20 nearby meteors and freezes them solid. Frozen meteors cannot be hurt by KINETIC attacks, Comets and Geodes ignore the cold, and ships are only slowed (Storm Titans half as much).',
  art: {
  ...
  paths: [
    {
      name: 'Deep Freeze',
      upgrades: [
        { name: 'Deep Chill', cost: 200, desc: 'Freezes last 0.6 s longer.',
        ...
        { name: 'Permafrost', cost: 1300, desc: 'Pulses fire 15% faster, and thawed meteors stay slowed to 50% for 2.5 s (ships slowed to 50% as well).',
          apply(s) {
            const a = main(s);
        ...
        { name: 'Glacial Core', cost: 4500, desc: 'Pulse radius +30, freezes last 0.8 s longer, each pulse freezes up to 20 more meteors, and ships are slowed to 40%.',
          apply(s) {
            const a = main(s);
    ...
    },
    {
      name: 'Embrittle',
      upgrades: [
        { name: 'Brittle Frost', cost: 260, desc: 'Everything the pulse hits turns brittle for 1.5 s, taking 1 extra damage from every hit.',
          apply(s) { const a = main(s); setHit(a, { brittle: { add: 1, mult: 1, t: 1.5 } }); } },
        { name: 'Deep Cracks', cost: 500, desc: 'Pulses deal 1 more damage and brittleness lasts 3 s.',
          apply(s) { const a = main(s); a.damage += 1; setHit(a, { brittle: { ...a.onHit.brittle, t: 3 } }); } },
        { name: 'Shatter Field', cost: 1400, desc: 'Towers within 160 units can hit frozen meteors with KINETIC attacks, and brittle targets take 2 extra damage per hit.',
          apply(s) {
            const a = main(s);
    ...
    },
    {
      name: 'Cryo Lance',
      upgrades: [
        { name: 'Cold Focus', cost: 230, desc: 'Pulse radius +20 and freezes last 0.3 s longer.',
        ...
        { name: 'Glacier Lance', cost: 6000, desc: 'Lances fire 20% faster, deal 6 more damage (41 more to ships), pierce 8 and slow ships to 35% for 3 s, and the emitter gains detection.',
          apply(s) {
            const a = main(s);
            s.detection = true;
