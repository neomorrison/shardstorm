...
export default {
  ...
  desc: 'Towers whose center is inside the 165 unit aura get 10% more range and attack 20% faster. Buffs from several Beacons do not stack: each tower keeps the best value of each buff.',
  art: {
  ...
  base: {
    ...
    attacks: {},
    aura: {
      radius: 165, rangeMult: 1.1, rateMult: 1.2, pierceAdd: 0, damageAdd: 0, shipDamageAdd: 0,
  ...
  paths: [
    {
      name: 'Radar',
      upgrades: [
        { name: 'Long-Range Scan', cost: 360, desc: 'Towers inside the aura get 20% more range.',
          apply(s) { atLeast(s, 'rangeMult', 1.2); } },
        { name: 'Sensor Net', cost: 720, desc: 'Towers inside the aura gain detection and can target Phantom meteors.',
          apply(s) { aura(s).detection = true; } },
        { name: 'Phase Radar', cost: 1800, desc: 'Radar pings strip Phantom from meteors inside a 230 unit aura so every tower can hit them, and towers inside get 25% more range.',
          apply(s) { grow(s, 65); atLeast(s, 'rangeMult', 1.25); addRadar(s, false); } },
        { name: 'Target Painter', cost: 10000, desc: 'The aura grows to 260 units, towers inside get 25% more range, and their hits deal 1 more damage to ships.',
          apply(s) { grow(s, 30); atLeast(s, 'rangeMult', 1.25); atLeast(s, 'shipDamageAdd', 1); } },
        { name: 'Omniscient Array', cost: 44000, desc: 'A 380 unit aura gives towers inside 40% more range and 5 more damage to ships, and strips Phantom from everything inside, Specter ships included.',
    ...
    },
    {
      name: 'Overclock',
      upgrades: [
        { name: 'Overclock', cost: 400, desc: 'Towers inside the aura attack 30% faster.',
    ...
    },
    {
      ...
      upgrades: [
        ...
        { name: 'Logistics Hub', cost: 2400, desc: 'Towers bought or upgraded inside a wider 185 unit aura cost 15% less.',
